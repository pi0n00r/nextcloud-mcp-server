...
<mkcalendar xmlns="urn:ietf:params:xml:ns:caldav" xmlns:d="DAV:" xmlns:cs="http://calendarserver.org/ns/">
    ...
    def _availability_timezone(name: Any) -> dt.tzinfo:
        ...
        local time on an unknown name. That is right for storing an event, but
        wrong here: business hours and preferred times need a known zone. An
        omitted zone uses UTC rather than freezing the server's current DST
        offset across the whole search window.
        """
    ...
    async def find_availability(
        ...
        ``timezone`` raises; malformed entries in ``preferred_times`` are
        skipped when at least one valid range remains, while a wholly invalid
        constraint raises. Overlapping entries are merged.
        ...
        busy = busy_spans_from_events(
            events,
            tz=tz,
            include_all_day=bool(constraints.get("include_all_day", True)),
        )
